"""Version-detection shims for Django 4.x – 6.x and Python 3.10 – 3.13+.
...
def assert_django_compatible() -> None:
    ...
    if DJANGO_VERSION < (4, 2):
        raise ImproperlyConfigured(
            f"django-activitylog-jwt requires Django >= 4.2. "
            f"You are running Django {'.'.join(str(v) for v in DJANGO_VERSION)}."
        )
    if PYTHON_VERSION < (3, 10):
        raise ImproperlyConfigured(
            f"django-activitylog-jwt requires Python >= 3.10. "
